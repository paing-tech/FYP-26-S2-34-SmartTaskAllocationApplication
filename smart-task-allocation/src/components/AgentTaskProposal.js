...
export default function AgentTaskProposal({ taskProposal, agentName }) {
  ...
  return (
    <div className="max-w-[90%] rounded-2xl bg-white/95 p-4 text-sm text-[#0D1E4C] shadow-[0_10px_30px_rgba(0,0,0,0.15)]">
      <p className="mb-2 text-xs font-bold uppercase tracking-wide text-[#0D1E4C]/60">Proposed tasks</p>
        ...
        {taskProposal.tasks.map((task, index) => (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-[#0D1E4C]/70">{task.description}</p>
            </div>
          </label>
        ))}
      </div>
      ...
      {stage === "select" ? (
        <button
          ...
          className="mt-3 h-9 w-full rounded-full bg-[#0D1E4C] text-xs font-black uppercase tracking-wide text-white transition hover:bg-[#0a1838] disabled:opacity-40"
        >
          Continue with {selected.size} task{selected.size === 1 ? "" : "s"}
        </button>
      ) : null}

      {stage === "approval" ? (
        <div className="mt-3 space-y-2">
          <p className="text-xs font-semibold text-[#0D1E4C]/80">Do these tasks need approval before they go live?</p>
          {error ? <p className="text-xs font-medium text-red-600">{error}</p> : null}
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => handleApprovalChoice(true)}
              className="h-9 flex-1 rounded-full border border-[#0D1E4C]/20 text-xs font-black uppercase tracking-wide text-[#0D1E4C] transition hover:bg-[#0D1E4C]/5"
            >
              Yes, need approval
            </button>
            <button
              ...
              className="h-9 flex-1 rounded-full bg-emerald-600 text-xs font-black uppercase tracking-wide text-white transition hover:bg-emerald-700"
            >
              No, auto-approve
            </button>
          </div>
