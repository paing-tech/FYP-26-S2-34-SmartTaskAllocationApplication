...
export default function MarketingContentManager() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
          ...
          {dirtyKeys.length ? (
            <>
              ...
              <button
                ...
                className="rounded-full border border-[#2563EB] bg-white/70 px-4 py-2 text-sm font-bold text-[#0D1E4C] transition hover:bg-white"
              >
                Preview draft
              </button>
            </>
      ...
      {isPreviewOpen ? (
        <div className="fixed inset-0 z-100 overflow-y-auto bg-black/70 p-4" role="dialog" aria-modal="true" aria-label="Draft site preview">
          <div className="mx-auto max-w-7xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="sticky top-0 z-10 flex items-center justify-between bg-[#0D1E4C] px-5 py-3 text-white">
              <div><strong>Draft preview</strong><p className="text-xs text-white/70">Private preview — public content has not changed.</p></div>
              <button type="button" onClick={() => setIsPreviewOpen(false)} className="rounded-full bg-white/15 px-4 py-2 text-sm font-bold">Close preview</button>
            </div>
            {tab === "homepage" ? (
              <div>
                {!draftByKey.nav.hidden ? <NavPreview draft={draftByKey.nav} onChange={() => {}} /> : null}
                {!draftByKey.hero.hidden ? <HeroPreview draft={draftByKey.hero} onChange={() => {}} /> : null}
                {!draftByKey.features.hidden ? <FeaturesPreview draft={draftByKey.features} onChange={() => {}} /> : null}
                {!draftByKey.testimonials_section.hidden ? <TestimonialsPreview draft={draftByKey.testimonials_section} onChange={() => {}} /> : null}
                {!draftByKey.footer.hidden ? <FooterPreview draft={draftByKey.footer} onChange={() => {}} /> : null}
              </div>
            ) : (
              <div>
                {!draftByKey.nav.hidden ? <NavPreview draft={draftByKey.nav} onChange={() => {}} /> : null}
                {!draftByKey.pricing.hidden ? <PricingPreview draft={draftByKey.pricing} onChange={() => {}} /> : null}
              </div>
            )}
          </div>
        </div>
