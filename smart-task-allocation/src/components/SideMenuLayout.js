...
function SideMenuLayoutInner({ actor, children }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!accessToken) {
        if (!cancelled) router.replace(`/login?returnTo=${encodeURIComponent(pathname)}`);
        return;
      }
      ...
      if (!routeResponse.ok || routeResult.homeRoute !== expectedHomeRoutes[actor]) {
        if (!cancelled) router.replace(routeResponse.ok && routeResult.homeRoute ? routeResult.homeRoute : "/login");
        return;
      }
      ...
      if (!cancelled && res.ok && result.agent) setAgentAvatarKey(result.agent.avatar_key);
    })().catch(() => {
  ...
  if (accessState !== "allowed") {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#C7DDEB] text-[#07183b]">
        <p className="rounded-full border border-white/60 bg-white/50 px-5 py-3 text-sm font-bold shadow-sm backdrop-blur">
          Verifying access...
        </p>
      </main>
    );
