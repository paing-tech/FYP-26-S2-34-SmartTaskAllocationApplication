...
import { validateTaskAssignee, validateTaskSchedule } from "@/lib/taskEligibility";
...
async function pickAiAssigneeForTask(supabase, { organizationId, taskId }) {
  ...
  for (const employeeId of employeeIds) {
    const eligibilityError = await validateTaskAssignee(supabase, { taskId, userId: employeeId, organizationId });
    if (eligibilityError) continue;
...
async function autoAllocateOptimusTasks(supabase, { organizationId }) {
  ...
  for (const task of candidateTasks) {
    ...
    for (const employeeId of employeeIds) {
      const eligibilityError = await validateTaskAssignee(supabase, {
        taskId: task.task_id,
        userId: employeeId,
        organizationId,
      });
...
export async function POST(request) {
  try {
    ...
    const scheduleError = validateTaskSchedule(startDatetime, endDatetime, { rejectPast: true });
    if (scheduleError) {
    ...
    if (assignedTo) {
      const eligibilityError = await validateTaskAssignee(supabase, {
        task: { organization_id: organizationId, start_datetime: startDatetime || null, end_datetime: endDatetime || null },
        userId: assignedTo,
        organizationId,
        requiredSkillIds,
      });
...
export async function PATCH(request) {
  try {
    ...
    if (action === "ai-assign-task") {
      ...
      const { data: filledPosition } = await supabase.from("task_assignee").select("user_id").eq("task_id", taskId).limit(1).maybeSingle();
      const { data: taskMirror } = await supabase.from("task").select("assigned_to").eq("task_id", taskId).maybeSingle();
      if (filledPosition || taskMirror?.assigned_to) {
        return NextResponse.json({ error: "This task position is already fully allocated." }, { status: 400 });
      }
      if (!organizationId) {
        return NextResponse.json({ error: "Organization ID is required." }, { status: 400 });
      }
      ...
      const eligibilityError = await validateTaskAssignee(supabase, { taskId, userId: match.employeeId, organizationId });
      if (eligibilityError) return NextResponse.json({ error: eligibilityError }, { status: 400 });
    ...
    if (assignedTo !== undefined) {
      if (assignedTo) {
        const organizationId = await getManagerOrganizationId(supabase, user);
        const eligibilityError = await validateTaskAssignee(supabase, { taskId, userId: assignedTo, organizationId });
        if (eligibilityError) return NextResponse.json({ error: eligibilityError }, { status: 400 });
      }
...
export async function DELETE(request) {
  try {
    ...
    const { data: legacyAssignment } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    if (activeAssignment || legacyAssignment) {
      return NextResponse.json(
        { error: "Cancel, complete, or reassign active employee assignments before deleting this task." },
        { status: 409 },
      );
    }
