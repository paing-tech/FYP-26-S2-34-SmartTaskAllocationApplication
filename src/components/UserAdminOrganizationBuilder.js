...
export default function UserAdminOrganizationBuilder() {
  ...
  async function submitOrganization(event) {
    ...
    try {
      ...
      setForm(emptyForm);
      setMessage(organization ? "Organization profile updated successfully." : "Organization profile created successfully.");
    } catch (saveError) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl">
      ...
      {message ? (
        <p className="mx-6 mt-5 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">
          {message}
        </p>
      ) : null}
      ...
      {isLoading ? (
        ...
        <div className="relative min-h-0 flex-1">
          <button
            ...
            className="absolute right-6 top-5 z-20 rounded-full bg-[#0a2a66] px-5 py-2.5 text-sm font-bold text-white shadow-lg hover:bg-[#061a40]"
          >
