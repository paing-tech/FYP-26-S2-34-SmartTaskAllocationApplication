  ...
  try {
    ...
    if (contentKey === "pricing") {
      ...
      for (const plan of plans) {
        const normalizedPrice = String(plan?.price ?? "").replace(/[^0-9.-]/g, "");
        const numericPrice = Number(normalizedPrice);
        if (!String(plan?.name ?? "").trim()) {
          return NextResponse.json({ error: "Each subscription plan requires a name." }, { status: 400 });
        }
        if (!normalizedPrice || !Number.isFinite(numericPrice) || numericPrice < 0) {
          return NextResponse.json(
            { error: `Enter a valid non-negative numeric price for ${plan.name || "the plan"}.` },
            { status: 400 },
          );
        }
      }
